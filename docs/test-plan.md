...
以上5B数字保持历史含义。5C最终完整复验为后端323（162单元/161集成）、前端87、浏览器37（原26+新11），全部退出0、无skip/only、无自动重跑。三套类型检查、两端构建及真实进程HTTP冒烟也均退出0。具体时间及失败记录见[5C验证](stage-5c-validation.md)。

| 编号 | 5C实际覆盖 | 层次/结果 |
|---|---|---|
| S5-02/04/16–18 | runtime.spec.ts同场双观察者、首页加入、两场并行、停止隔离、重复结束、总结降级 | E通过；不以多观察者替代两场并行 |
| S5-05/12/14 | 原runner回归；浏览器运行未结束时已有发言/观点，来源覆盖边界；保留旧综合失败由原store/前端呈现测试覆盖 | U/I/E通过对应行为；真实语义Q未执行 |
| S5-22 | sse.test.ts快照间隙与实时提交；events.test.ts同version整批；runtime.spec.ts真实半批中断恢复 | I/U/E通过 |
| S5-23 | sse.test.ts header优先/400/reset/终态重放；discussion-stream有限GET恢复与定时器清理；浏览器离线终态恢复 | I/U/E通过，去重及旧快照防覆盖 |
| S5-24 | sse.test.ts两观察者/跨场；controller选择token；runtime.spec.ts双观察者/两场/切换/刷新 | I/U/E通过，观察不增加Fake调用 |
| S5-25 | sse.test.ts真实HTTP心跳/终态end/204/订阅清理；sse-backpressure.test.ts真实SQLite+受控响应write=false/drain/10秒关闭 | I/U通过；背压传输故障注入，不冒充大规模慢网络负载测试 |
| S5-26 | Studio SSR/控制测试；runtime.spec.ts运行/收尾/完成/中断/总结不可用、1/4/8专家、三视口、键盘/阅读位置 | U/E通过；代表截图实际查看，未做移动真机全套可访问性审计 |
| S5-27 | 全部原后端/前端与26浏览器回归；已确认阶段性文案替换为真实开始入口 | 全部通过，未删除旧核心约束 |
| S5-28 | 原Fake外网拦截回归；测试后端fetch全拒绝；事件白名单/严格parser/文本呈现；静态私有配置忽略检查 | U/I/E及静态检查通过对应范围；无真实调用、无敏感原值读取 |
