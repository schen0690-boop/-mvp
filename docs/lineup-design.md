...
状态：**待用户确认的完整设计提案，尚未冻结为实施授权**。基线03cf8ea；本轮只读核查和Markdown变更，无业务代码、迁移执行、依赖安装或模型调用。用户P5要求优先于旧设计；正文中具体字段、路径和恢复机制为C类推荐，确认后才用于4B。不是writing-plans实施任务表。
...
该方案的重建顺序来自官方说明，**本轮未执行或证明具体迁移SQL通过**。当前项目无业务触发器/视图；预检遇未知对象应停止而非丢弃。重跑已知002只校验版本/指纹/外键，不重建、不改applied_at或业务版本。升级成功后原POST/GET路径、旧草稿DTO值、幂等创建和排序保留；服务/读取类型需扩展才能读取新状态，不以旧只读草稿代码冒充完成迁移兼容。
