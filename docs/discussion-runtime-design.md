# 讨论执行、调度与实时事件契约（5A设计 / 5B实施）

5A设计日期：2026-09-16，基线`1c8acef`。本文是唯一核心规格；HTTP/SSE字段在contracts，验收矩阵在test-plan。下文原始调研/待确认措辞保留5A时点语境；以本节5B确认记录及[实际验证报告](stage-5b-validation.md)区分当前实现，不将5C计划当已完成。

## 5B确认与实施记录（P11，2026-09-16）

用户已确认总体设计、completed+summary.unavailable、串行综合检查点、内容有效性优先的公平协调、最多两次额外主持介入，以及原预算推导。B(N)=28N+56覆盖1–8专家，两次总结包含在内。实际003、四类Fake能力、runner、HTTP和事件持久化通过本轮测试；真实模型/SSE/演播厅未实现。

P11明确允许最小前端兼容，覆盖原第11节“运行解析仅留5C”的划分：5B已有严格24字段解析和安全状态标签，仍不增加开始/结束或讨论展示区。原三组待确认决定在本轮已获答复，不再重复请求批准。

公平细化：等待/成功次数以成功提交计算；有效申请排序后优先尝试非连续第三次的候选，若这些候选全部发言生成失败，可由原连续发言者兜底，防止只剩一个可用角色时卡死。不是无条件轮转。调用超时后的迟到结果须符合run/epoch/source及原期限；数据库写事务末尾到期也回滚发言/观点/成功总结及事件。

实现文件对应关系及旧数据兼容结果见architecture/验证报告；001/002未改变、正式库和4D验收库未迁移。5B没有额外提交广播钩子，5C须基于已提交public_events增加通知和真实传输，不假定已有内存事件平台。
