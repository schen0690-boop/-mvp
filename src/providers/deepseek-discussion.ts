...
export interface DiscussionMetric {operation:DiscussionOperation;taskId:string;attempt:number;elapsedMs:number;httpStatus?:number;finishReason?:string;usage:Record<string,number>|'未取得';outcome:string;requestedModel:string;responseModel?:string;startedAt:string;validResult:boolean}
export class DeepSeekDiscussionProvider implements DiscussionProvider {
 ...
 private async call<T>(operation:DiscussionOperation,input:DiscussionInput,context:DiscussionContext,parse:(raw:unknown)=>T):Promise<T>{
  const messages=discussionMessages(operation,input,context);let metric:RequestMetric|undefined;let validResult=false;const startedAt=new Date().toISOString();
  try{
   ...
   const result=parse(raw);validResult=true;return result;
  }catch(error){if(metric&&error instanceof DiscussionValidationError)metric.outcome=error.kind;throw error;}
  finally{if(metric){const safe:DiscussionMetric={operation,taskId:context.taskId,attempt:context.attemptNo,elapsedMs:metric.elapsedMs,usage:metric.usage??'未取得',outcome:metric.outcome,requestedModel:this.config.model,startedAt,validResult};if(metric.responseModel)safe.responseModel=metric.responseModel;if(metric.httpStatus!==undefined)safe.httpStatus=metric.httpStatus;if(metric.finishReason)safe.finishReason=metric.finishReason;try{this.record(safe);}catch{/* Diagnostics cannot change the result. */}}}
 }
