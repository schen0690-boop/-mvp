...
it('first domain-valid real-adapter result closes authorization; another call never reaches transport',async()=>{
  ...
  const provider=new DeepSeekRosterProvider({baseUrl:'https://api.deepseek.com',model:'deepseek-flash',apiKey:'guard-dummy',maxTokens:4096},async()=>{calls++;expect(a.count).toBe(1);return Response.json({choices:[{index:0,finish_reason:'stop',message:{role:'assistant',content:raw}}]});},m=>a.record(a.count,m));
