...
it('演播厅区分总结不可用和业务失败，保留公开正文，不渲染内部JSON',()=>{
 ...
 expect(html).toContain('讨论已结束，但总结生成失败。');expect(html).toContain('真实已提交发言。');expect(html).toContain('Fake');expect(html).not.toContain('sourceTranscriptVersion');expect(html).not.toContain('>开始讨论<');
});
