...
const root='evidence/stage-5c',stamp=new Date().toISOString().replaceAll(':','-'),raw=root+'/raw';
...
const report={stage:'5C',provider:'Fake only; historical adapter tests use local stubs/injected transport',codeRevision:git(['rev-parse','HEAD']),workingChanges:git(['status','--short']),startedAt:new Date().toISOString(),commands:[]};
...
if(report.commands.length===commands.length&&report.commands.every(c=>c.exitCode===0)){
 ...
 report.results={backend:backend.numPassedTests,backendUnit:count('unit'),backendIntegration:count('integration'),backendPending:backend.numPendingTests,frontend:frontend.numPassedTests,frontendPending:frontend.numPendingTests,e2e:e2e.stats};
