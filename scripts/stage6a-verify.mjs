...
const root='evidence/stage-6a',stamp=new Date().toISOString().replaceAll(':','-'),raw=root+'/raw';
...
const report={stage:'6A',provider:'Real discussion adapter via local HTTP only; default regression Fake',officialRequests:0,codeRevision:git(['rev-parse','HEAD']),workingChanges:git(['status','--short']),startedAt:new Date().toISOString(),commands:[]};
for(const [name,args] of commands){
 console.log('Verifying: '+name);const startedAt=new Date().toISOString();let stdout='',stderr='';
 const exitCode=await new Promise((resolve,reject)=>{
  const child=spawn(process.execPath,args,{windowsHide:true,stdio:['ignore','pipe','pipe'],env:{...process.env,E2E_EVIDENCE_ROOT:'evidence/stage-6a'}});
