...
mkdirSync('.tmp/stage-6b',{recursive:true});const root=mkdtempSync(resolve('.tmp/stage-6b/dry-')),file=join(root,'test.sqlite');await initializeConfiguredDatabase(file);const db=openConfiguredDatabase(file);let blockedExternal=0,loopbackRequests=0;const nativeFetch=globalThis.fetch;
/** @type {typeof fetch} */
const loopbackOnly=async(url,init)=>{const endpoint=new URL(String(url));if(endpoint.protocol!=='http:'||endpoint.hostname!=='127.0.0.1'){blockedExternal++;throw Error('NON_LOCAL_MODEL_REQUEST_BLOCKED');}loopbackRequests++;return nativeFetch(url,init);};
globalThis.fetch=loopbackOnly;
// The stub captures only the loopback-guarded fetch, never unrestricted fetch.
const stub=await startDiscussionStub();globalThis.fetch=async()=>{throw Error('EXTERNAL_NETWORK_FORBIDDEN');};
...
const binding={authorizationId:randomUUID(),discussionId:id,runId:randomUUID(),generationId:g.generationId,lineupRevision:1};const auth=new DiscussionAuthorization(join(root,'authorization'),binding);writeFileSync(join(root,'prepared.json'),JSON.stringify({...binding,codeRevision:'local-dry-run',testOnly:true}));
...
let closing=false;const stop=()=>{if(closing)return;closing=true;clearInterval(watch);auth.close('shutdown');server.closeAllConnections();server.close(()=>{void Promise.all([discussion.close(),lineup.close()]).finally(async()=>{await stub.close();writeFileSync(join(root,'local-network-proof.json'),JSON.stringify({testOnly:true,provider:'local HTTP stub',loopbackRequests,blockedExternal,officialRequests:0,privateConfigLoaded:false,closedAuthorization:auth.closed,counts:auth.counts}));db.close();process.disconnect?.();});});};process.on('message',m=>{if(m==='shutdown')stop();});process.once('SIGTERM',stop);process.once('SIGINT',stop);
